<template>
  <div class="add-post-container" :class="{'sticky-add-post': !isArticle ? isAddPostFocused.state || postContent.trim() !== '' : true}">
    <div class="post-overlay-before" :class="{'post-overlay': !isArticle ? isAddPostFocused.state : true}" @click="postFocus(false)"></div>
    <div class="add-post box-nue" :class="{'add-post-focused': !isArticle ? isAddPostFocused.state : true}" @click.stop="postFocus(true)">
        ...
        <div class="full-width add-post-selection-block">
          ...
          <div class="cat-loc-wrapper full-width flex h-start">
            <div class="set-category-wrapper flex v-center">
              <div class="category none-clickable-label" :class="{'validate-error': isEmptyCategory}" v-if="breadcrumb.length === 0" @click.stop="getCategorySelectionModal">Category*<span class="dim"> (required)</span></div>
              <ul class="selected-breadbrumb clickable-label" v-if="breadcrumb.length > 0" @click.stop="getCategorySelectionModal">
                <li class="inline-flex v-center" v-show="item.selected" v-for="(item, index) in breadcrumb" :key="index">
                  <span>{{item.name_en}}</span>
                </li>
              </ul>
            </div>
            <div class="set-category-wrapper flex v-center">
              <div class="category none-clickable-label" v-if="locationBreadcrumb.length === 0" @click.stop="getLocationSelectionModal">Location<span class="dim"> (optional)</span></div>
              <ul class="selected-breadbrumb selected-location-breadbrumb clickable-label" v-if="locationBreadcrumb.length > 0" @click.stop="getLocationSelectionModal">
                <li class="inline-flex v-center" v-show="item.selected" v-for="(item, index) in locationBreadcrumb" :key="index">
                  <span>{{item.name_en}}</span>
                </li>
              </ul>
            </div>
      ...
      <div class="body-row">
        <p v-if="!isArticle ? !isAddPostFocused.state : false" class="input-placeholder" @click="postFocus(true)">{{typePlaceholder}}</p>
        <!-- {{content.postContent}} -->
        <textarea v-show="!isArticle && isAddPostFocused.state" ref="addPostTextarea" class="add-post-textarea"
          :class="{'content': postContent.trim() !== '', 'add-post-textarea-focused': isPostFocused2}"
          v-autosize="postContent"
          v-model="postContent"
          :placeholder="typePlaceholder"
        />
        <textarea v-show="isArticle" ref="addPostTextareaNew" class="add-post-textarea"
          :class="{'content': postContent.trim() !== '', 'add-post-textarea-focused': isPostFocused2}"
          v-autosize="postContent"
      ...
      <div class="add-post-foot-row full-width flex v-center h-between">
        <ul class="add-post-actions">
          <li v-if="!isArticle">
            <div class="input-charm-wrapper">
              <input type="file" @change="addMedia(1, $event)" multiple class="input-charm input-charm-photo" id="input-charm-photo" accept=".gif, .jpg, .jpeg, .png"/>
        ...
        <Button class="pull-right" :label="!isArticle ? 'Publish' : 'Update'" :action="post" :loading="isPosting"/>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    displayName() {
      let displayName = ''
      if (this.isProfileDataLoaded) {
        displayName = this.profileData.displayName ? this.profileData.displayName : this.user.providerData[0].displayName
      }
      return displayName
    }
  },
  methods: {
    ...
    addMedia(type, event) {
      if (type === 1) {
        event.target.files.forEach(o => {
          if (o.type === 'image/png' || o.type === 'image/jpeg' || o.type === 'image/gif') {
            this.files.push(o)
          } else {
    ...
    uploadAttachments(file,post_id,index){
      return new Promise((resolve,reject) => {
        ...
        //Upload the file
        let task = this.fb.storage().ref().child(basePath).put(file)
    ...
    postToDB () {
      ...
      if (this.user) {
        this.postContent
        let canPost = this.filesCompressed.length === 0 && this.postContent.trim() === '' ? false : true
        if (canPost) {
          this.db.collection("User_Posts").add({
              ...
              contactOption: this.user.phoneNumber != null ? this.user.phoneNumber : this.user.email
            }).then(doc => {
              this.postContent = ''
              this.isPostFocused2 = false
              ...
              this.files = []
              this.eventBus.$emit('message', {msg: 'Your post has been successfully published', type: 0})
          ...
          this.eventBus.$emit('message', {msg: 'You cannot publish empty contents', type: 1})
          this.isPosting = false
        }
      } else {
        this.eventBus.$emit('message', {msg: 'You are not authorized', type: 1})
        this.isPosting = false
      }
    },
    updateDB () {
      ...
      if (this.user) {
        this.postContent
        let canPost = this.filesCompressed.length === 0 && this.content.postContent.trim() === '' ? false : true
        if (canPost) {
          ...
          if (self.files.length === self.compressedCounter) {
            if (self.isArticle) {
              ...
              if (self.breadcrumb.length === 0) {
                ...
                self.eventBus.$emit('message', {msg: 'You cannot publish without selecting a category', type: 1})
              } else {
                self.postToDB()
              }
            }
    ...
    post() {
      ...
      if (this.breadcrumb.length === 0) {
        ...
        this.isPosting = false
        this.eventBus.$emit('message', {msg: 'You cannot publish without selecting a category', type: 1})
      } else {
        ...
        if (this.files.length === 0) {
          if (this.isArticle) {
            ...
            this.postLocation = JSON.parse(JSON.stringify(this.locationBreadcrumb))
    ...
    setPostCategory() {
      ...
      if (category.childs && category.childs.length > 0) {
        ...
        deselected_child_list.sort(function(a, b){return b-a});
