<template>
  <div class="post box-nue" :class="postType">
    ...
    <div class="body-row">
      <carousel class="post-carousel" :perPage="1" v-if="data.ImageCount > 0 && imageUrls.length > 0">
      ...
      <div class="flex m-t-2">
        <ul class="post-meta-tags">
          <li class="post-meta meta-type" :class="data.postType === 1 ? 'consumer' : 'seller'">
            {{data.postType === 1 ? 'Consumer' : 'Freelancer'}}
          </li>
        </ul>
        <ul class="selected-breadbrumb none-clickable-label m-l-1" v-if="categoryHierarchy.length > 0">
          <li class="inline-flex v-center" v-show="item.selected" v-for="(item, index) in categoryHierarchy" :key="index">
    ...
    <div class="header-row">
      ...
      <div class="user-head-block">
        <router-link :to="'/'+findyoName(data.userName)+'/timeline'" class="user-title">{{data.userName}}</router-link>
        <!-- <p class="user-subtitle">UI Engineer</p> -->
      </div>
      <div class="post-details-block">
        <p class="posted-time">{{postedTime}}</p>
        <p class="text-right post-from" v-if="endLocation && endLocation.trim() !== ''" v-text="'from ' + endLocation"></p>
      </div>
    </div>
    <div class="foot-row">
      <post-action :likeAction="like" :isLike="isLiked" :likesCount="data.likes.length" :repliesCount="data.numberOfComments" :replyAction="isExpand ? reply : undefined" :isReply="isReply" :shareAction="share" :toggleReplyAction="expand" :expanded="openReplies" :viewLikesActions="viewLikedUsers" :contactAction="showContactModal" :isMine="isMine" :data="data"/>
    </div>
    <div class="textarea-wrapper" v-if="isReply || !isExpand">
      <!-- <textarea class="content" v-autosize="replyMsg" v-model="replyMsg"/> -->
      <div contenteditable="" ref="commentDiv" class="contenteditable content" @keydown.enter.prevent @keyup="updateReplyMsg" @paste="pasteContent($event)"></div>
    ...
    <add-post class="post-edit-window" :eventBus="eventBus" v-if="(user ? true : false) && isEditable.state" :user="user" :db="db" :postTypes="postTypes" :isAddPostFocused="isAddPostFocused" :isArticle="isArticle" :isEditable="isEditable" :content="data" :imageUrls="imageUrls" :breadcrumb="breadcrumb" :nestedCats="nestedCats" :nestedLocs="nestedLocs" :locationBreadcrumb="locationBreadcrumb" :isCategorySelectModal="isCategorySelectModal" :isLocationSelectModal="isLocationSelectModal" :profileData="profileData" :isProfileDataLoaded="isProfileDataLoaded"/>
    <stateleess-modal v-if="isDeleteModal" :ok="okDelete" :okLabel="'Yes, delete'" :cancelLabel="'Cancel'" :close="closeDelete">You want to delete this post?</stateleess-modal>
    <stateleess-modal v-if="isContactTypeModal" :okLabel="'Yes, delete'" :cancelLabel="'Cancel'" :close="closeContactModal">
      <div>
...
export default {
  ...
  methods: {
    ...
    getImageUrl() {
      ...
      for (let x = 1; x <= this.data.ImageCount; x++) {
        storageRef.child(`Posts/${this.data.id}/${x}.jpg`).getDownloadURL().then(url => {
    ...
    reply (obj) {
      if (obj) {
        ...
        setTimeout(() => {
          this.$refs.commentDiv.innerHTML = `<span ref="replyToUser" class="reply-to-user">@${this.findyoName(this.replyToUser.displayName)}</span>`
        }, 0)
      } else {
    ...
    setLike(state) {
      ...
      axios.post(process.env.VUE_APP_ROOT_API + '/postLike', {
        like: state,
        postID: this.data.id,
        userID: user.uid
      })
    ...
    postComment () {
      if (this.isLoggedIn) {
        ...
        if (comment.length >= 100) {
          this.eventBus.$emit('message', {msg: 'Maximum character limit is 100', type: 1})
        } else if (!!this.replyMsg === false) {
          this.eventBus.$emit('message', {msg: 'Type your reply before send', type: 1})
        } else {
          let self = this
          ...
          axios.post(process.env.VUE_APP_ROOT_API + '/postComment', commentObject)
    ...
    contact() {
      ...
      this.$router.push({'path': `/chat/${this.findyoName(this.data.userName)}`})
    },
    showContactModal() {
      if (this.isLoggedIn) {
  ...
  mounted () {
    ...
    if (this.data.location) {
      ...
      this.endLocation = this.flatternLocations.length > 0 ? this.flatternLocations[this.flatternLocations.length - 1] : ''
    }
  }
};
</script>
