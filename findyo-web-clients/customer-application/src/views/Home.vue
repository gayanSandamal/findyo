<template>
  <div class="page home">
    <div class="feed-wrapper">
      <categorySelectModal v-if="isCategorySelectModal.state" :list="nestedCats" :breadcrumb="categoryHierarchy" :isCategorySelectModal="isCategorySelectModal"/>
      <locationSelectModal v-if="isLocationSelectModal.state" :list="locations.locations" :breadcrumb="locationHierarchy" :isLocationSelectModal="isLocationSelectModal"/>
      <add-post :eventBus="eventBus" v-if="user ? true : false" :db="db" :auth="auth" :user="user" :fb="fb" :postTypes="postTypes" :isAddPostFocused="isAddPostFocused" :breadcrumb="categoryHierarchy" :nestedCats="nestedCats" :nestedLocs="locations.locations" :locationBreadcrumb="locationHierarchy" :isCategorySelectModal="isCategorySelectModal" :isLocationSelectModal="isLocationSelectModal" :profileData="userData" :isProfileDataLoaded="isProfileDataLoaded"/>
      <ul v-if="posts.length > 0 && !isZeroPosts" class="feed-main-filter-block box-nue flex v-center" :class="{'feed-main-filter-block-sticky': !isAddPostFocused.state}">
        <li class="filter-icon-wrapper">
          <span class="label-charm filter-icon"></span>
        </li>
        <li class="feed-main-filter-icon-all switch-nue" :class="[{'selected': 0 === selectedType.typeId}, postTypeLabel(0)]" @click.stop="filterByType(0)">
          All
        </li>
        <li class="switch-nue"
      ...
      <ul v-else class="feed-main-filter-block box-nue" :class="{'feed-main-filter-block-sticky': !isAddPostFocused.state}">
        <li class="filter-icon-wrapper">
      ...
      <ul class="feed-main-posts-block" v-if="posts.length > 0 && !isZeroPosts">
        <li v-for="post in posts" :key="post.id">
          <Post :data="post" :user="user" :postTypes="postTypes" :isAddPostFocused="isAddPostFocused" :eventBus="eventBus" :db="db" :auth="auth" :breadcrumb="categoryHierarchy" :nestedCats="nestedCats" :nestedLocs="locations.locations" :locationBreadcrumb="locationHierarchy" :isCategorySelectModal="isCategorySelectModal" :isLocationSelectModal="isLocationSelectModal" :profileData="userData" :isProfileDataLoaded="isProfileDataLoaded"/>
        </li>
      </ul>
      <div v-if="!isMorePostsLoading" @click.stop="loadPosts" class="button flex v-center h-center m-b-2">Load more posts</div>
      ...
      <div class="home-sidebar home-sidebar-2">
        <div class="box-nue">
          ...
          <ul class="filter-breadcrumb">
            <li v-for="(item, idx) in locationBreadcrumb" :key="idx">{{title(item)}}<span class="tag-remove" @click.stop="removeLoc(item)"></span></li>
          </ul>
          <location-list :list="locationList" :languageId="languageId" :addedFilters="locationBreadcrumb" :isParent="true"/>
          <div class="full-width">
...
export default {
  ...
  components: {
    ...
    'location-list': () => import ('@/components/common/Lists/LocationList'),
    ...
    locationSelectModal: () => import("@/components/common/Modals/LocationSelection")
  },
  computed: {
    locationList() {
      ...
      if (postsFilter !== 0) {
        ...
        .then(querySnapshot => {
          ...
          if (querySnapshot.docs.length > 0) {
            this.lastSnapshotChkObj2 = (querySnapshot.docs[querySnapshot.docs.length - 1]).data()
          } else {
